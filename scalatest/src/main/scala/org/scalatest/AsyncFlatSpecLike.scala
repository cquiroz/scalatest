...
trait AsyncFlatSpecLike extends AsyncSuite with AsyncTestRegistration with ShouldVerb with MustVerb with CanVerb { thisSuite =>
  ...
  final def registerTest(testText: String, testTags: Tag*)(testFun: => Any): Unit = {
    ...
    // SKIP-SCALATESTJS-END
    //SCALATESTJS-ONLY val stackDepthAdjustment = -4
    engine.registerAsyncTest(testText, () => PastOutcome(OutcomeOf.outcomeOf(testFun)), Resources.testCannotBeNestedInsideAnotherTest, "AsyncFlatSpecLike.scala", "registerTest", 4, stackDepthAdjustment, None, None, testTags: _*)
  }
  ...
  final def registerIgnoredTest(testText: String, testTags: Tag*)(testFun: => Any): Unit = {
    ...
    // SKIP-SCALATESTJS-END
    //SCALATESTJS-ONLY val stackDepthAdjustment = -4
    engine.registerIgnoredAsyncTest(testText, () => PastOutcome(OutcomeOf.outcomeOf(testFun)), Resources.testCannotBeNestedInsideAnotherTest, "AsyncFlatSpecLike.scala", "registerIgnoredTest", 4, stackDepthAdjustment, None, testTags: _*)
  }
