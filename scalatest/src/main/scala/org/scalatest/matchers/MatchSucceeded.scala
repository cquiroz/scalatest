...
object MatchSucceeded {
   ...
   */
  def unapply(matchResult: MatchResult): Option[Prettifier => String] =
    if (matchResult.matches) Some((prettifier: Prettifier) => matchResult.negatedFailureMessage(prettifier)) else None
}
